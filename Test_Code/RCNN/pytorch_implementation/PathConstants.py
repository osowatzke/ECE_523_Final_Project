...
class PathConstants():
    ...
    def __new__(cls, use_local_copy=False):
        if not cls._instance:
            super().__new__(cls)
            cls._PATH_THIS_FILE = os.path.dirname(__file__)
    ...
    def __set_data_dir(cls):
        if cls._use_local_copy:
            cls.DATA_DIR = '/tmp/FLIR_ADAS_v2'
        else:
            cls.DATA_DIR = os.path.join(cls._PATH_THIS_FILE,'..','..','..','FLIR_ADAS_v2')
